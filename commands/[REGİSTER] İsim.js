...
//HINDIRI BINDIRI
    ...
    if(!isim) return message.lineReply("Geçerli bir isim belirtmelisin!").then(x => x.delete({timeout: 5000}))
        ...
        if(user.roles.cache.has(client.config.TagRolRegister)) {
            ...
            await user.setNickname(`✰ ${isim.charAt(0).toUpperCase() + isim.slice(1).toLowerCase()}${yas ? ` | ${yas}` : ``}`).catch(e => { });
            
            message.lineReply(RegisterEmbed.setDescription(`
            ${user} kişisininin ismini başarılya "✰ ${isim.charAt(0).toUpperCase() + isim.slice(1).toLowerCase()}${yas ? ` | ${yas}` : ``}" yaptınız.
           
            ${client.emojis.cache.find(x => x.name === "wex_carpi")}  Kişinin toplamda ${kayıt.length} kayıtı bulundu.
            ${isimler}
            ...
            await user.setNickname(`✰ ${isim.charAt(0).toUpperCase() + isim.slice(1).toLowerCase()}${yas ? ` | ${yas}` : ``}`).catch(e => { });
            message.lineReply(RegisterEmbed.setDescription(`
            ${user} kişisi "✰ ${isim.charAt(0).toUpperCase() + isim.slice(1).toLowerCase()}${yas ? ` | ${yas}` : ``}" ismiyle Kadın olarak kayıt edildi.
