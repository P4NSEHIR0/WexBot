...
exports.execute = async (client, message, args) => {
  let vmutebilgi = await db.get(`vmutebilgi_wex`);
  let cpuanbilgi = await db.get(`cpuanbilgi_wex`);
...
qdb.push(`info.${user.id}.kullanıcıvoice`, {
  ...
  cezatip: "Voice Mute "
    ...
    jdb.push(`sessusturulma`, {id: user.id, kalkmaZamani: Date.now()+ms(time)})

    }, ms(time));
  
    
  let wexdmembed = new MessageEmbed().setAuthor(message.author.tag, message.author.avatarURL({dynamic: true})).setFooter(`Ceza Numaran: #${Cezaİd} & CezaPuanın: ${cpuan}`).setColor("RANDOM");
