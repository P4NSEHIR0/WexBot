...
\`•\` [**3. Yetki**]  ${await db.has(`3üncücyt_${message.guild.id}`) ? `(${ücüncüyt.map(c => `<@&${c}>`)})` : `\`.rolesetup 3yetki @rol @rol2 @rol3\``}
    ...
    let collector = await msg.createButtonCollector(filter, {time: 50000})
      ...
      if(button.id === "1") {
        ...
        client.channels.cache.get("926900275682361434").send(embed.setDescription(`${user} adlı kullanıcı başlangıç yetkisinden ${message.author} tarafından başlatıldı.
        Verilen Yetkiler: ${baslangıcyt.map(c => `<@&${c}>`)}`))
      }
      if(button.id === "2") {
        await button.reply.defer()
        msg.delete()
        ...
        client.channels.cache.get("926900275682361434").send(embed.setDescription(`${user} adlı kullanıcı 2. yetkiden ${message.author} tarafından başlatıldı.
        Verilen Yetkiler: ${ikinciyt.map(c => `<@&${c}>`)}`))
        message.channel.send(embed.setDescription(`${user} adlı kullanıcıya ${await db.has(`baslangıcyt_${message.guild.id}`) ? `(${ikinciyt.map(c => `<@&${c}>`)})` : `\`Roller belirtilmemiş\``} rolleri verildi.`))
      }
      if(button.id === "3") {
        await button.reply.defer()
        msg.delete()
        ...
        client.channels.cache.get("926900275682361434").send(embed.setDescription(`${user} adlı kullanıcı 3. yetkiden ${message.author} tarafından başlatıldı.
        Verilen Yetkiler: ${ücüncüyt.map(c => `<@&${c}>`)}`))
        message.channel.send(embed.setDescription(`${user} adlı kullanıcıya ${await db.has(`baslangıcyt_${message.guild.id}`) ? `(${ücüncüyt.map(c => `<@&${c}>`)})` : `\`Roller belirtilmemiş\``} rolleri verildi.`))
      }
