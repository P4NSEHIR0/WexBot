...
exports.execute = async (client, message, args) => {
  ...
  if(!["926191916574453770", "926806184743292958", "924684545763205140", "926811647346102282"].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.lineReply("NE YAPMAYA ÇALIŞIYORSUN APTAL.")
 ...
 let banbilgi = await db.get(`banbilgi_wex`);

    let KaraListe = qdb.fetch(`karaL.${message.author.id}`)
    if(KaraListe) return message.react(client.emojis.cache.find(x => x.name === "wex_carpi"));
  ...
  qdb.set(`user.${user.id}.sicil`, {
    ...
    yetkili: message.author.id, 
    uyes: user.id,     
     yetkili2: message.author.userName,
    bsure: tarih,
    ssure: "Sınırsız",
    cezatip: "Ban",
    cpuan : "10"
...
qdb.push(`info.${user.id}.kullanıcıvoice`, {
sebep: reason,
kod: Cezaİd,
yetkili: message.author.id, 
uyes: user.id,
...
cezatip: "Ban "
   ...
   user.ban({reason: `${reason} | ${message.author.tag}`}).catch(err => console.error(error));
    ...
    if(await qdb.get(`banL_${message.author.id}`) >= client.config.banLimit) {
      qdb.add(`karaLP_${message.author.id}`, +1)
      message.member.roles.set(client.config.BanLimitCezalı)
    }
...
exports.conf = {
  ...
  aliases: ["yasakla", "wexinlaneti", "gavur"] // Komutumuzun yardımcıları
}