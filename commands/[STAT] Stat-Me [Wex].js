...
if(button.id === "3") {
  await button.reply.defer()
  message.channel.send(embed.setDescription(`
  ${client.emojis.cache.find(x => x.name === "wex_nokta")} Taglı Üye: ${taglı}
  ${client.emojis.cache.find(x => x.name === "wex_nokta")} Davet: (Toplam ${total}) (Regular ${regular})
  ${client.emojis.cache.find(x => x.name === "wex_nokta")} Kayıt: (Toplam ${toplamkayit || "0"}) (Erkek,${erkek} - Kadın,${kız})
  ${client.emojis.cache.find(x => x.name === "wex_nokta")} Kayıt Market Puanı: ${mkp} puan. (.kayıtmarket)   
  `))
}
