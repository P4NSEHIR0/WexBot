...
client.channels.cache.find(a => a.name === "cmd-log").send(`[\`${moment(+Date.now()).format(`Do MMMM YYYY | HH:mm`)}\`] - \`${message.author.tag}\` adlı kullanıcı <#${message.channel.id}> kanalında \`.profil\` komutunu kullandı. [\`${message.content}\`]`);

}


exports.conf = {
  command: "profil", // Asıl komutumuz
