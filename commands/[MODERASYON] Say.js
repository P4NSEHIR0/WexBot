...
module.exports.execute = async (client, message, args) => {       
  ...
  if (message.member.permissions.has(8) || message.member.roles.cache.some(e => arr.some(x => x == e))) {
   ...
   let w = "Achilles"
   kırkk = "§"
   let isimsayı = message.guild.members.cache.filter(u => u.user.username.includes("✰")).size;
   var etiket =  message.guild.members.cache.filter(u => u.user.username).filter(member => member.user.discriminator == "0020").size;
