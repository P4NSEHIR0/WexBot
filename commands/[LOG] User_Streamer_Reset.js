...
const wexembed = new Discord.MessageEmbed().setFooter(`Sıfırlanmadan önce açtıgı yayın sayısı: ${yayınacmauser} `).setColor("BLUE").setDescription(`${csm} (\`${csm.id}\`) adlı kullanıcının yayın açma geçmisi ${message.author} \`(${message.author.id})\` tarafından sıfırlandı.`)
client.channels.cache.get(client.config.StreamerDenetleyiciLog).send(wexembed);
