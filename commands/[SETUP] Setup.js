...
exports.execute = async (client, message, args) => {
  ...
  if(message.author.id !== "569158273601896461") return message.react(client.emojis.cache.find(x => x.name === "wex_carpi"));
    ...
    \`\`\`SERVER\`\`\`
    ${await db.has(`tag1_wex_${message.guild.id}`) ? `[**${acik}**] **TAG 1.** (\`${await db.get(`tag1_wex_${message.guild.id}`)}\`)` : `[**${kapali}**] \`.serversetup tag1 <TAG>\`[**SERVER 1. TAG**]`}
    ${await db.has(`tag2_wex_${message.guild.id}`) ? `[**${acik}**] **TAG 2.** (\`${await db.get(`tag2_wex_${message.guild.id}`)}\`)` : `[**${kapali}**] \`.serversetup tag2 <TAG>\`[**SERVER 2. TAG**]`}
    ...
    \`\`\`KANAL\`\`\`
    ${await db.has(`registerchat_wex`) ? `[**${acik}**] **Register Chat** (<#${await db.get(`registerchat_wex`)}>)` : `[**${kapali}**] \`.kanalsetup kayıtkanal #kanal\`[**Register-Chat**]`}
    ${await db.has(`vmutebilgi_wex`) ? `[**${acik}**] **VoiceMute Bilgi** (<#${await db.get(`vmutebilgi_wex`)}>)` : `[**${kapali}**] \`.kanalsetup vmutebilgi #kanal\`[**Vmute-Bilgi**]`}
    ${await db.has(`cmutebilgi_wex`) ? `[**${acik}**] **ChatMute Bilgi** (<#${await db.get(`cmutebilgi_wex`)}>)` : `[**${kapali}**] \`.kanalsetup cmutebilg #kanal\`[**CMute-Bilgi**]`}
    ${await db.has(`jailbilgi_wex`) ? `[**${acik}**] **Jail Bilgi** (<#${await db.get(`jailbilgi_wex`)}>)` : `[**${kapali}**] \`.kanalsetup jailbilgi #kanal\`[**Jail-Bilgi**]`}
    ${await db.has(`reklambilgi_wex`) ? `[**${acik}**] **Reklam Bilgi** (<#${await db.get(`reklambilgi_wex`)}>)` : `[**${kapali}**] \`.kanalsetup reklambilgi #kanal\`[**Reklam-Bilgi**]`}
    ${await db.has(`banbilgi_wex`) ? `[**${acik}**] **Ban Bilgi** (<#${await db.get(`banbilgi_wex`)}>)` : `[**${kapali}**] \`.kanalsetup banbilgi #kanal\`[**Ban-Bilgi**]`}
    ${await db.has(`cpuanbilgi_wex`) ? `[**${acik}**] **Cezapuan Chat** (<#${await db.get(`cpuanbilgi_wex`)}>)` : `[**${kapali}**] \`.kanalsetup cezapuanbilgi #kanal\`[**Cezapuan-Bilgi**]`}
    ${await db.has(`kurallar_wex`) ? `[**${acik}**] **Kurallar Kanalı** (<#${await db.get(`kurallar_wex`)}>)` : `[**${kapali}**] \`.kanalsetup kurallar #kanal\`[**Cezapuan-Bilgi**]`}
    ...
    \`\`\`ROLLER BOT\`\`\`
    ${await db.has(`etkinlikkatılım_wex`) ? `[**${acik}**] **Etkinlik Katılımcısı**: (${etkinlikkatılım.map(c => `<@&${c}>`)})` : `[**${kapali}**] \`.rolesetup etkinlikkatılımcısı @rol\``}
    ${await db.has(`cekiliskatılım_wex`) ? `[**${acik}**] **Çekiliş Katılımcısı**: (${cekiliskatılım.map(c => `<@&${c}>`)})` : `[**${kapali}**] \`.rolesetup cekiliskatılımcısı @rol\``}
