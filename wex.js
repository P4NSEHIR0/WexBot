...
let gün = moment(Date.now()).format("DD")
let saat = moment(Date.now()).format("HH:mm:ss")
const WexEmbed = new Discord.MessageEmbed().setFooter(`${gün}/${aylar[moment(Date.now()).format("MM")]}/${yıl} ${saat}`).setColor("BLUE").setDescription(`
...
`);
client.channels.cache.find(a => a.name === "nickname-log").send(WexEmbed);
    })
...
let saat = moment(Date.now()).format("HH:mm:ss")
  ...
  const WexEmbed = new Discord.MessageEmbed().setColor("BLUE").setDescription(`
...
const { botOwner, botPrefix, botToken, guildID, botVoiceChannelID, inviteChannelID, durum } = require('./wexab.js');
const mongoose = require('mongoose');
mongoose.connect('mongodb+srv://wex:wex@wex.xtauw.mongodb.net/wex?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});// Mongo connect linki
...
client.on('message', async (message) => {
  ...
  if (message.content.startsWith('.wexrolal')) {
    message.delete()
...
client.login("OTI3NzA4Nzc0ODcyMDE4OTY0.YdOKHw.W6B_R2UC3B5UN-Na7pl63m_GW1w").then(c => console.log(chalk `{cyan Connected_Bot } Bot: {magenta ${client.user.tag} }`)).catch(err => console.error(`Bot giriş yapamadı!`));  
client2.login("OTI2ODkzNjQ5MzUzMTEzNjcw.YdCS-g.U_qARymBadWhpaFHKKIpCnM4uXQ"); //.env ye gir
